...
```bash
herdr completion zsh    # generate zsh completion script
```
...
```bash
herdr completion zsh > ~/.herdr_completion.zsh
```

Source the completion script in your `~/.zshrc`:

```bash
# Add this line to ~/.zshrc
source ~/.herdr_completion.zsh
```
...
After installation, you can use tab completion with herdr commands:

```bash
herdr <tab>           # list all available commands
herdr workspace <tab> # list workspace subcommands
herdr agent start --<tab>  # list available flags
```
